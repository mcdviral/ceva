...
MatrixXd computeBlendingMatrix(int N, bool cummulative)
{
    MatrixXd m(N, N);
    m.setZero();
...
class PoseAnalyticFactorX : public ceres::CostFunction
{
public:

    PoseAnalyticFactorX(const SE3d &pose_meas_, double wR_, double wP_, int N_, double Dt_, double s_)
        : pose_meas(pose_meas_), wR(wR_), wP(wP_), N(N_), Dt(Dt_), s(s_)
    {
        ...
        MatrixXd B = computeBlendingMatrix(N, false);

        // Blending matrix for rotation, in cummulative form
        MatrixXd Btilde = computeBlendingMatrix(N, true);
